...
These are files in your own repo, so they render as long as GitHub renders.
    ...
    python scripts/cards.py --user lestarhenriquesss-pixel --out assets
...
def render_stats(user, stats, theme):
    ...
    out = [
        ...
        f'fill="{c["muted"]}">visão geral</text>',
    ...
    return frame(W, H, c, "".join(out), f"Estatísticas do GitHub de {user}")
...
def render_repo(repo, theme):
    ...
    desc = repo.get("description") or "Sem descrição disponível."
...
def main(argv=None):
    ...
    tiles = [("Total de estrelas", f"{stars:,}"),
             ("Repositórios públicos", f"{user['public_repos']:,}"),
             ("Seguidores", f"{user['followers']:,}")]

    contrib = fetch_contributions(args.user, token)
    if contrib:
        total, current, longest = contrib
        tiles += [("Contribuições (1 ano)", f"{total:,}"),
                  ("Sequência atual", f"{current:,}"),
                  ("Maior sequência", f"{longest:,}")]
    else:
